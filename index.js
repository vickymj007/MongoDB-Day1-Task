...
//List the four product which are grater than 500 in price
//We have only 3 products which are grater than 500 in price
...
db.products.find().forEach(product => {
    print(`${product.product_name}, ${product.product_material}`)
});
...
db.products.aggregate([
    {$group:{_id:'$product_price', duplicates:{$addToSet:'$_id'},count:{$sum:1}}},
    {$match:{count:{$gt:1}}}
]).forEach(product=>{
    product.duplicates.shift()
    db.products.deleteMany({_id:{$in:product.duplicates}})
})
